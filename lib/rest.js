...
class BinanceRest {
    ...
    constructor({
        ...
        this.onlyCreateRequest = onlyCreateRequest;
    ...
    _makeRequestCall(options, cb) {
        const handlingFunction = (err, response, body) => {
        ...
        if (this.requestFunction === request) {
            request(options, (err, response, body) => {
                handlingFunction(err, response, body);
            });
        } else {
            //URL, parameters, headers
            this.requestFunction(options.url, {}, options.headers, (response) => {
    ...
    _makeRequest(query, callback, route, security, method, attempt = 0) {
        ...
        if (security === 'SIGNED') {
            ...
            if ('onlyCreateRequest' in queryCopy) {
                delete queryCopy['onlyCreateRequest']
            }
            queryString = qs.stringify(queryCopy);
        ...
        if (query.onlyCreateRequest) {
            ...
            const action = cb => {
                this._makeRequestCall(options, cb);
    ...
    _setOnlyCreateRequest(query) {
        if (!query.onlyCreateRequest) {
            query.onlyCreateRequest = this.onlyCreateRequest;
        }
    }
    ...
    trades(query = {}, callback) {
        ...
        this._setOnlyCreateRequest(query);
